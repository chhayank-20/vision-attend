...
export function EnrollmentManager() {
  ...
  if (isLoading)
    return <div className="p-4 text-slate-400">Loading requests...</div>
  if (!requests?.length)
    return (
      <div className="bg-slate-50 p-12 rounded-[2rem] border border-dashed border-slate-200 text-center space-y-2">
        ...
        <p className="text-xs text-slate-300">
          Remote requests will appear here for your approval.
        </p>
      </div>
    )
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {requests.map((req) => (
        <div
          key={req.id}
          className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col group transition-all hover:shadow-md"
        >
          <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
            {/* Note: In a real app, you'd serve the image via an authenticated endpoint */}
            <img
              src={`/api/users/face-image?path=${req.image_path}`}
